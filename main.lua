...
function Wallabag2:onDispatcherRegisterActions()
    ...
    Dispatcher:registerAction("wallabag_folder", { category="none", event="OpenWallabag2", title=_("Open Wallabag dir"), general=true,})
    Dispatcher:registerAction("toggle_filter_tag", {category = "none", event = "ToggleFilterTag", title = _("Wallabag : Toggle Filter Tag"), general = true,})
end
...
function Wallabag2:onSynchronizeWallabag2()
    local connect_callback = function()
        self:synchronize()
        self:refreshCurrentDirIfNeeded()
        self.ui:handleEvent(Event:new("OpenWallabag2"))
...
function Wallabag2:onOpenWallabag2()
    if self.ui.document then
        self.ui:onClose()
    end
    if FileManager.instance then
...
function Wallabag2:onToggleFilterTag()
    if self.filter_tag == "p" then
        self.filter_tag = ""  -- Set to empty string if currently "p"
    else
        self.filter_tag = "p"  -- Set to "p" if currently empty
    end
    self:saveSettings()  -- Save the updated filter tag
    UIManager:show(InfoMessage:new{
        text = T(_("swapped article tag to: '%1'"), self.filter_tag),
        timeout = 1,
     })
end
